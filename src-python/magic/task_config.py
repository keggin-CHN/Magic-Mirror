...
def canonicalize_video_task_config(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    segment_overlap_frames = _coerce_non_negative_int(
        payload.get('segmentOverlapFrames')
    )
    ...
    *,
    ...
    if parsed is not None:
        if (
            ...
            and parsed.get('schema') == 'magicmirror.video-task'
        ):
...
def _parse_signed_token(
    config_id: str, *, prefix: str, secret: str
) -> Optional[Dict[str, Any]]:
